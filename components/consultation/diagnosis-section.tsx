...
export function DiagnosisSection({ department, data, onChange }: DiagnosisSectionProps) {
  ...
  // Log when data is received for debugging - only when data actually changes and component isn't initialized
  useEffect(() => {
  ...
  const addDiagnosis = useCallback(
    (diagnosis: string) => {
      if (!safeData.includes(diagnosis)) {
        const newData = [...safeData, diagnosis]
        memoizedOnChange(newData)
      }
    },
    [safeData, memoizedOnChange],
  )

  const removeDiagnosis = useCallback(
    (diagnosis: string) => {
      const newData = safeData.filter((d) => d !== diagnosis)
      memoizedOnChange(newData)
    },
    [safeData, memoizedOnChange],
  )
  ...
  const addCustomDiagnosis = useCallback(() => {
    if (customDiagnosis.trim() && !safeData.includes(customDiagnosis.trim())) {
      const newData = [...safeData, customDiagnosis.trim()]
      memoizedOnChange(newData)
      setCustomDiagnosis("")
    }
  }, [customDiagnosis, safeData, memoizedOnChange])
