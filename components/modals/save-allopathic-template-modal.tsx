...
export function SaveAllopathicTemplateModal({
  ...
  const { saveAllopathicTemplate, updateAllopathicTemplate } = usePrescriptionTemplate()
  ...
  const handleSave = async () => {
    ...
    const validMedicines = medicines.filter((med) => med.medicine.trim())
    ...
    try {
      ...
      let success = false
      if (editingTemplate) {
        success = await updateAllopathicTemplate(editingTemplate.id, templateData)
      } else {
        success = await saveAllopathicTemplate(templateData)
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{editingTemplate ? "Edit Allopathic Template" : "Save Allopathic Template"}</DialogTitle>
        </DialogHeader>
